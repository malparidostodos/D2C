    ...
    this.material = new RawShaderMaterial({
      ...
      fragmentShader: `
          ...
          void main() {
            ...
            #elif MODE == 4
            // Modo transparente con colores sutiles
            vec4 spectralColor = spectral(mix(340.0, 700.0, lum));
            // Hacer los colores muy sutiles: mezclar con blanco para reducir saturación
            vec3 subtleColor = mix(vec3(1.0), spectralColor.rgb, 0.15);
            // Usar la luminancia para el alpha (transparencia)
            // Ajustar la curva para que sea más suave y menos obvia
            float alpha = pow(lum, 0.5) * 0.6;
            gl_FragColor = vec4(subtleColor, alpha);
            #endif
          }`,
      depthTest: false,
      depthWrite: false,
